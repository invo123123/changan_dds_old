...
*****************************************************************/
#ifndef CAR_CONTROL_FR_PROXY_HPP
#define CAR_CONTROL_FR_PROXY_HPP
...
namespace com {
	namespace changan {
		namespace carControl {
			namespace proxy {
				extern void on_carControl_fr_rpc_reply_data_available(int32_t reader, void* arg);
				extern void on_carControl_fr_notify_data_available(int32_t reader, void* arg);
				...
				template <typename T>
				void on_CarControlFRProxy_request_publication_matched_cb(dds_entity_t writer, const dds_publication_matched_status_t  status, void* arg) {
					ASF_LOG_DEBUG_ARGS("on_CarControlFRProxy_request_publication_matched_cb, writer=[%d], current_count=[%d]", writer, status.current_count);
					ProviderMatchedStatus data = DDSUtil::get_provider_matched_status(writer, status);
					//printf("on_CarControlFRProxy_request_publication_matched_cb, data.is_online[%d]777777\n",data.is_online);
					if (data.is_online) 
					{
				...
				template <typename T>
                void on_CarControlFRProxy_reply_sublication_matched_cb(dds_entity_t reader, const dds_subscription_matched_status_t status, void* arg) {
					ASF_LOG_DEBUG_ARGS("on_CarControlFRProxy_reply_sublication_matched_cb, reader=[%d], current_count=[%d]", reader, status.current_count);
                    ConsumerMatchedStatus data = DDSUtil::get_consumer_matched_status(reader, status);
                    //printf("on_CarControlFRProxy_reply_sublication_matched_cb 8888888888 data.is_online[%d]8888888\n",data.is_online);
					if (data.is_online) {
                        T::get_instance().add_link_count();
                    }
				...
				template <typename T>
				bool on_CarControlFRProxy_request_topicdata_checkable_cb(dds_entity_t entity, const uint8_t guid[DDS_GUID_LENGTH], const unsigned char* topicdata_value, uint32_t topicdata_len, void* arg) {
					ASF_LOG_DEBUG_ARGS("on_CarControlFRProxy_request_topicdata_checkable_cb");
					//printf("on_CarControlFRProxy_request_topicdata_checkable_cb 999999999\n");
					if (topicdata_len <= 0) {
						ASF_LOG_DEBUG_ARGS("on_CarControlFRProxy_request_topicdata_checkable_cb, topicdata_len %u <= 0", topicdata_len);
				...
				class CarControlFRProxy:public dds_link_count
				{
				public:
				...
				protected:
					...
					CarControlFRProxy();
					CarControlFRProxy(const CarControlFRProxy& signal);
					const CarControlFRProxy& operator=(const CarControlFRProxy& signal);
