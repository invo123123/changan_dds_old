...
namespace proxy {
	...
	Cnr_TPMSWarn_Proxy::Cnr_TPMSWarn_Proxy(){

    }

	bool Cnr_TPMSWarn_Proxy::init(std::string config) {
	...
	void Cnr_TPMSWarn_Proxy::wait_server_online()
	{
		CarControlFLProxyGw::get_instance().wait_server_online();
	}
	Msg<TireWarnInfo> Cnr_TPMSWarn_Proxy::Get_TireWarnInfo( size_t dds_rpc_time_out) {
		Msg<TireWarnInfo> _rpc_result;
		...
		{
			ASF_LOG_ERROR_ARGS("Cnr_TPMSWarn_Proxy::Get_TireWarnInfo dds_get_guid faild rc=[%d]", rc);
		...
		_rpc_request->data._d = carControl_Car_Control_Cnr_TPMSWarn_Get_TireWarnInfo_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_TPMSWarn_Proxy::Get_TireWarnInfo pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_TPMSWarn_Get_TireWarnInfo_Hash, _rpc_client_sn.low);
		...
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_TPMSWarn_Get_TireWarnInfo.return_;
	...
	Cnr_TPMSWarn_Proxy& Cnr_TPMSWarn_Proxy::get_instance()
	{
		static Cnr_TPMSWarn_Proxy instance;
		return instance;
	}

	bool Cnr_TPMSWarn_Proxy::is_init(){
