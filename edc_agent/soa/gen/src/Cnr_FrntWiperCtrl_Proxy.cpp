...
namespace proxy {
	...
	Cnr_FrntWiperCtrl_Proxy::Cnr_FrntWiperCtrl_Proxy(){

    }

	bool Cnr_FrntWiperCtrl_Proxy::init(std::string config) {
	...
	void Cnr_FrntWiperCtrl_Proxy::wait_server_online()
	{
		CarControlFLProxyGw::get_instance().wait_server_online();
	}
	Msg<ResponseInfo> Cnr_FrntWiperCtrl_Proxy::RR_WiperCtrl(const uint8_t &WiperCtrl,const RequestInfo &RequestInfo_, size_t dds_rpc_time_out) {
		...
		{
			ASF_LOG_ERROR_ARGS("Cnr_FrntWiperCtrl_Proxy::RR_WiperCtrl dds_get_guid faild rc=[%d]", rc);
		...
		_rpc_request->data._u.Cnr_FrntWiperCtrl_RR_WiperCtrl.WiperCtrl = WiperCtrl;
		_rpc_request->data._u.Cnr_FrntWiperCtrl_RR_WiperCtrl.RequestInfo = RequestInfo_;

		_rpc_request->data._d = carControl_Car_Control_Cnr_FrntWiperCtrl_RR_WiperCtrl_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_FrntWiperCtrl_Proxy::RR_WiperCtrl pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_FrntWiperCtrl_RR_WiperCtrl_Hash, _rpc_client_sn.low);
		...
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_FrntWiperCtrl_RR_WiperCtrl.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	Msg<OperationInfo> Cnr_FrntWiperCtrl_Proxy::Get_SrvOperInfo( size_t dds_rpc_time_out) {
		...
		if (DDS_RETCODE_OK != rc)
		{
			ASF_LOG_ERROR_ARGS("Cnr_FrntWiperCtrl_Proxy::Get_SrvOperInfo dds_get_guid faild rc=[%d]", rc);
			_rpc_result.code = DDS_RPC_REMOTE_EX_UNSUPPORTED;
			return _rpc_result;
		}
		...
		_rpc_request->data._d = carControl_Car_Control_Cnr_FrntWiperCtrl_Get_SrvOperInfo_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_FrntWiperCtrl_Proxy::Get_SrvOperInfo pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_FrntWiperCtrl_Get_SrvOperInfo_Hash, _rpc_client_sn.low);
		auto _rpc_future = CarControlFLProxyGw::get_instance().call(_rpc_client_sn.low, _rpc_request);
		carControl_Car_Control_Request_free(_rpc_request, DDS_FREE_ALL);
		...
		_rpc_result.code = _rpc_ret->header.remoteEx;
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_FrntWiperCtrl_Get_SrvOperInfo.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	...
	Cnr_FrntWiperCtrl_Proxy& Cnr_FrntWiperCtrl_Proxy::get_instance()
	{
		static Cnr_FrntWiperCtrl_Proxy instance;
		return instance;
	}

	bool Cnr_FrntWiperCtrl_Proxy::is_init(){
