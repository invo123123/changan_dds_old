...
namespace proxy {
	...
	Cnr_PowerShiftSwt_Proxy::Cnr_PowerShiftSwt_Proxy(){

    }

	bool Cnr_PowerShiftSwt_Proxy::init(std::string config) {
	...
	void Cnr_PowerShiftSwt_Proxy::wait_server_online()
	{
		CarControlFLProxyGw::get_instance().wait_server_online();
	}
	Msg<ResponseInfo> Cnr_PowerShiftSwt_Proxy::RR_PwrShiftSwt(const uint8_t &PowerCmd,const RequestInfo &RequestInfo_, size_t dds_rpc_time_out) {
		...
		{
			ASF_LOG_ERROR_ARGS("Cnr_PowerShiftSwt_Proxy::RR_PwrShiftSwt dds_get_guid faild rc=[%d]", rc);
		...
		_rpc_request->data._u.Cnr_PowerShiftSwt_RR_PwrShiftSwt.PowerCmd = PowerCmd;
		_rpc_request->data._u.Cnr_PowerShiftSwt_RR_PwrShiftSwt.RequestInfo = RequestInfo_;

		_rpc_request->data._d = carControl_Car_Control_Cnr_PowerShiftSwt_RR_PwrShiftSwt_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_PowerShiftSwt_Proxy::RR_PwrShiftSwt pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_PowerShiftSwt_RR_PwrShiftSwt_Hash, _rpc_client_sn.low);
		...
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_PowerShiftSwt_RR_PwrShiftSwt.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	Msg<OperationInfo> Cnr_PowerShiftSwt_Proxy::Get_SrvOperInfo( size_t dds_rpc_time_out) {
		...
		if (DDS_RETCODE_OK != rc)
		{
			ASF_LOG_ERROR_ARGS("Cnr_PowerShiftSwt_Proxy::Get_SrvOperInfo dds_get_guid faild rc=[%d]", rc);
			_rpc_result.code = DDS_RPC_REMOTE_EX_UNSUPPORTED;
			return _rpc_result;
		}
		...
		_rpc_request->data._d = carControl_Car_Control_Cnr_PowerShiftSwt_Get_SrvOperInfo_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_PowerShiftSwt_Proxy::Get_SrvOperInfo pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_PowerShiftSwt_Get_SrvOperInfo_Hash, _rpc_client_sn.low);
		auto _rpc_future = CarControlFLProxyGw::get_instance().call(_rpc_client_sn.low, _rpc_request);
		carControl_Car_Control_Request_free(_rpc_request, DDS_FREE_ALL);
		...
		_rpc_result.code = _rpc_ret->header.remoteEx;
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_PowerShiftSwt_Get_SrvOperInfo.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	...
	Cnr_PowerShiftSwt_Proxy& Cnr_PowerShiftSwt_Proxy::get_instance()
	{
		static Cnr_PowerShiftSwt_Proxy instance;
		return instance;
	}

	bool Cnr_PowerShiftSwt_Proxy::is_init(){
