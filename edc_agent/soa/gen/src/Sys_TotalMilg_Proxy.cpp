...
namespace proxy {
	...
	Sys_TotalMilg_Proxy::Sys_TotalMilg_Proxy(){

    }

	bool Sys_TotalMilg_Proxy::init(std::string config) {
	...
	void Sys_TotalMilg_Proxy::wait_server_online()
	{
		CarControlFLProxyGw::get_instance().wait_server_online();
	}
	Msg<uint32_t> Sys_TotalMilg_Proxy::Get_TotalMilg( size_t dds_rpc_time_out) {
		Msg<uint32_t> _rpc_result;
		...
		{
			ASF_LOG_ERROR_ARGS("Sys_TotalMilg_Proxy::Get_TotalMilg dds_get_guid faild rc=[%d]", rc);
		...
		_rpc_request->data._d = carControl_Car_Control_Sys_TotalMilg_Get_TotalMilg_Hash;
		ASF_LOG_DEBUG_ARGS("Sys_TotalMilg_Proxy::Get_TotalMilg pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Sys_TotalMilg_Get_TotalMilg_Hash, _rpc_client_sn.low);
		...
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Sys_TotalMilg_Get_TotalMilg.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	Msg<ReevTotalMilg> Sys_TotalMilg_Proxy::Get_ReevTotalMilg( size_t dds_rpc_time_out) {
		Msg<ReevTotalMilg> _rpc_result;
		...
		if (DDS_RETCODE_OK != rc)
		{
			ASF_LOG_ERROR_ARGS("Sys_TotalMilg_Proxy::Get_ReevTotalMilg dds_get_guid faild rc=[%d]", rc);
			_rpc_result.code = DDS_RPC_REMOTE_EX_UNSUPPORTED;
			return _rpc_result;
		}
		...
		_rpc_request->data._d = carControl_Car_Control_Sys_TotalMilg_Get_ReevTotalMilg_Hash;
		ASF_LOG_DEBUG_ARGS("Sys_TotalMilg_Proxy::Get_ReevTotalMilg pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Sys_TotalMilg_Get_ReevTotalMilg_Hash, _rpc_client_sn.low);
		auto _rpc_future = CarControlFLProxyGw::get_instance().call(_rpc_client_sn.low, _rpc_request);
		carControl_Car_Control_Request_free(_rpc_request, DDS_FREE_ALL);
		...
		_rpc_result.code = _rpc_ret->header.remoteEx;
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Sys_TotalMilg_Get_ReevTotalMilg.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	...
	Sys_TotalMilg_Proxy& Sys_TotalMilg_Proxy::get_instance()
	{
		static Sys_TotalMilg_Proxy instance;
		return instance;
	}

	bool Sys_TotalMilg_Proxy::is_init(){
