...
namespace proxy {
	...
	Cnr_WinAllCtrl_Proxy::Cnr_WinAllCtrl_Proxy(){

    }

	bool Cnr_WinAllCtrl_Proxy::init(std::string config) {
	...
	void Cnr_WinAllCtrl_Proxy::wait_server_online()
	{
		CarControlFLProxyGw::get_instance().wait_server_online();
	}
	Msg<OperationInfo> Cnr_WinAllCtrl_Proxy::Get_SrvOperInfo( size_t dds_rpc_time_out) {
		...
		{
			ASF_LOG_ERROR_ARGS("Cnr_WinAllCtrl_Proxy::Get_SrvOperInfo dds_get_guid faild rc=[%d]", rc);
		...
		_rpc_request->data._d = carControl_Car_Control_Cnr_WinAllCtrl_Get_SrvOperInfo_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_WinAllCtrl_Proxy::Get_SrvOperInfo pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_WinAllCtrl_Get_SrvOperInfo_Hash, _rpc_client_sn.low);
		...
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_WinAllCtrl_Get_SrvOperInfo.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	Msg<ResponseInfo> Cnr_WinAllCtrl_Proxy::RR_ObjDrv(const RequestInfo &RequestInfo_,const uint16_t &MotCtrlPos, size_t dds_rpc_time_out) {
		...
		if (DDS_RETCODE_OK != rc)
		{
			ASF_LOG_ERROR_ARGS("Cnr_WinAllCtrl_Proxy::RR_ObjDrv dds_get_guid faild rc=[%d]", rc);
			_rpc_result.code = DDS_RPC_REMOTE_EX_UNSUPPORTED;
			return _rpc_result;
		}
		...
		DDS_SequenceNumber_t _rpc_client_sn = com::changan::rpc_service::DDSClient::get_rpc_request_id();
		_rpc_request->header.requestId.sequence_number = _rpc_client_sn;
		_rpc_request->data._u.Cnr_WinAllCtrl_RR_ObjDrv.RequestInfo = RequestInfo_;
		_rpc_request->data._u.Cnr_WinAllCtrl_RR_ObjDrv.MotCtrlPos = MotCtrlPos;

		_rpc_request->data._d = carControl_Car_Control_Cnr_WinAllCtrl_RR_ObjDrv_Hash;
		ASF_LOG_DEBUG_ARGS("Cnr_WinAllCtrl_Proxy::RR_ObjDrv pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Cnr_WinAllCtrl_RR_ObjDrv_Hash, _rpc_client_sn.low);
		auto _rpc_future = CarControlFLProxyGw::get_instance().call(_rpc_client_sn.low, _rpc_request);
		carControl_Car_Control_Request_free(_rpc_request, DDS_FREE_ALL);
		...
		_rpc_result.code = _rpc_ret->header.remoteEx;
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Cnr_WinAllCtrl_RR_ObjDrv.return_;
			_rpc_result.autoFreePtr = sp;
		}
		return _rpc_result;
	}
	...
	Cnr_WinAllCtrl_Proxy& Cnr_WinAllCtrl_Proxy::get_instance()
	{
		static Cnr_WinAllCtrl_Proxy instance;
		return instance;
	}

	bool Cnr_WinAllCtrl_Proxy::is_init(){
