...
namespace proxy {
	...
	Basc_RLSSts_Proxy::Basc_RLSSts_Proxy(){

    }

	bool Basc_RLSSts_Proxy::init(std::string config) {
	...
	void Basc_RLSSts_Proxy::wait_server_online()
	{
		CarControlFLProxyGw::get_instance().wait_server_online();
	}
	Msg<RLSSts> Basc_RLSSts_Proxy::Get_WiperRLSSts( size_t dds_rpc_time_out) {
		Msg<RLSSts> _rpc_result;
		...
		{
			ASF_LOG_ERROR_ARGS("Basc_RLSSts_Proxy::Get_WiperRLSSts dds_get_guid faild rc=[%d]", rc);
		...
		_rpc_request->data._d = carControl_Car_Control_Basc_RLSSts_Get_WiperRLSSts_Hash;
		ASF_LOG_DEBUG_ARGS("Basc_RLSSts_Proxy::Get_WiperRLSSts pre call, hashId=[%d], reqId=[%d]", carControl_Car_Control_Basc_RLSSts_Get_WiperRLSSts_Hash, _rpc_client_sn.low);
		...
		if (_rpc_result.code == DDS_RPC_REMOTE_EX_OK) {
			_rpc_result.data = _rpc_ret->data._u.Basc_RLSSts_Get_WiperRLSSts.return_;
	...
	Basc_RLSSts_Proxy& Basc_RLSSts_Proxy::get_instance()
	{
		static Basc_RLSSts_Proxy instance;
		return instance;
	}

	bool Basc_RLSSts_Proxy::is_init(){
